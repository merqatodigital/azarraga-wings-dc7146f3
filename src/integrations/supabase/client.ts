...
function createSupabaseClient() {
  ...
  // Build/demo mode: the workspace must open directly, without an email/password gate.
  // Anonymous Supabase users still receive the authenticated Postgres role, so the existing
  // RLS policies, created_by provenance, commercial workflows and server-agent JWT forwarding work.
  if (typeof window !== 'undefined') {
    void client.auth.getSession().then(async ({ data, error }) => {
      if (!error && !data.session) {
        const { error: anonymousError } = await client.auth.signInAnonymously();
        if (anonymousError) console.error('[Supabase] Silent workspace session failed:', anonymousError.message);
      }
    });
  }
  return client;
