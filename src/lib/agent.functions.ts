...
function runtimeSecret(name: string): string | undefined {
  ...
  const globalEnv = (globalThis as any)?.env as Record<string, string | undefined> | undefined;
...
export const listAgentModels = createServerFn({ method: "GET" })
  ...
  .handler(async () => {
    try {
      ...
      const all = (json.data ?? []).map((m) => ({ id: String(m.id), name: String(m.name ?? m.id), contextLength: (m.context_length as number) ?? null, free: zero(m?.pricing?.prompt) && zero(m?.pricing?.completion) }));
...
export const askAgent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const key = runtimeSecret("OPENROUTER_API_KEY");
    if (!key) return { error: "OPENROUTER_API_KEY is not visible to the app runtime. Publish the Lovable app after saving the secret.", reply: "", model: data.model };
    ...
    const dbError = [evidence, quotes, invoices, leads, docs, customers].find((r: any) => r.error)?.error;
    if (dbError) return { error: `Commercial memory query failed: ${dbError.message}`, reply: "", model: data.model };
    const icmContext = JSON.stringify({ note: "All amounts are integer centavos in PHP. Historical unit prices are EVIDENCE ONLY.", commercialMemory: evidence.data ?? [], quotes: quotes.data ?? [], invoices: invoices.data ?? [], leads: leads.data ?? [], sourceDocuments: docs.data ?? [], customers: customers.data ?? [] });
    const system = `${SYSTEM_RULES}\nCurrent intent: ${data.intent}.\nICM CONTEXT (live database):\n${icmContext}`;
    try {
      const res = await fetch(OPENROUTER_CHAT, { method: "POST", headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json", "HTTP-Referer": "https://lovable.dev", "X-Title": "Azarraga Commercial Agent" }, body: JSON.stringify({ model: data.model, messages: [{ role: "system", content: system }, { role: "user", content: data.message }], temperature: 0.2 }) });
      const json = (await res.json()) as any;
      if (!res.ok) return { error: json?.error?.message ?? `OpenRouter returned ${res.status}`, reply: "", model: data.model };
      return { error: null as string | null, reply: String(json?.choices?.[0]?.message?.content ?? ""), model: data.model, humanReviewRequired: true };
    } catch (error) {
      console.error("[Azarraga Agent] OpenRouter request failed", error);
